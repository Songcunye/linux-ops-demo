...
### 实操现象说明
1. `/proc/* No such file or directory`：`/proc` 是内核虚拟文件系统，临时文件，可直接忽略；
2. 少量 `Permission denied`：系统私有保护目录，root 也无访问权限，不影响正常文件检索结果。

### 踩坑记录
普通用户执行全盘检索会大量输出权限拒绝报错，两种解决办法：
1. `su root` 切换管理员账号（本次实操使用）
2. `sudo find / -type f -size +100M` 临时提升权限执行
...
### 踩坑记录
1. sed 表达式与目标文件之间缺少空格，会报 `unknown option` 语法错误；
2. 目标文件不存在时，提示 `No such file or directory`。
...
```
### 管道分步拆解
1. `awk '{print $1}'`：以空格分割文本，提取每行第一个字段（网站访问日志一般存放客户端 IP）；
2. `sort`：对提取的字段进行排序；
3. `uniq -c`：统计重复内容出现次数，输出格式：`次数 对应内容`。

### 实操输出示例
文件内容：`warn test log`
执行命令后输出：`1 warn`，代表首字段 warn 仅出现 1 次。
...
# 权限问题总结
普通用户读取系统日志、检索根目录文件会触发 `Permission denied` 权限拒绝报错。
两种标准解决方案：
1. `su root` 切换完整管理员权限；
2. `sudo + 命令` 单次临时提升执行权限。
